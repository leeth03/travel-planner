...
"""
    ...
    # User input area
    st.subheader(f"Get {option}")
    user_prompt = st.text_area(
        "Enter your travel details or questions:",
        placeholder=example_prompts[option]["placeholder"],
        value=example_prompts[option]["default"],
        height=150,
    )
    ...
    if st.button("Generate Plan", type="primary"):
        if user_prompt:
            with st.spinner("Thinking..."):
                planner.process_request(system_prompts[option], user_prompt)
        else:
            st.warning("Please enter your travel details or questions.")
